...
from config import (bids_dir, deriv_dir, event_id, task,
                    task, preprocess_opts, bv_montage)
...
for sub in sub_list:
    ...
    raw_fif_file = deriv_sub_dir / \
        f'sub-{sub_id}_task-{task}_ref-FCz_desc-import_raw.fif.gz'
    ...
    event_file = deriv_sub_dir / f'{sub}_task-{task}_desc-resamp_eve.txt'
    ...
    json_file = deriv_sub_dir / f'{sub}_task-{task}_desc-resamp_eve.json'
    ...
    # Save RANSAC
    ransac_file = deriv_sub_dir / f'{sub}_task-{task}_ref_FCz_ransac.pkl'
    ...
    json_file = deriv_sub_dir / f'{sub}_task-{task}_ref_FCz_ransac.json'
    with open(json_file, 'w') as outfile:
        json.dump(json_info, outfile, indent=4)
    del json_info, json_file
    ...
    eog_ica_file = fig_sub_dir / f'{sub}_task-{task}_ic_eog_scores.png'
